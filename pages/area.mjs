import { fetchDataApi } from "../modules/api.mjs";
import { getAreaFromQueryString } from "../modules/area.mjs";
import { setChangeBackground, setData, setText } from "../modules/elements.mjs";
...
async function load(filters, fields, callback) {
  ...
  callback(getRecent(res.data, RECENT_DAYS));
}
...
load(
  "areaType=" +
    area.primary[1] +
    (area.primary[0] ? ";areaName=" + area.primary[0] : ""),
  [
  ...
  function (data) {
    ...
    load(
      "areaType=" +
        area.healthcare[1] +
        (area.healthcare[0] ? ";areaName=" + area.healthcare[0] : ""),
